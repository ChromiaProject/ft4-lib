...
describe("Transaction priority", () => {
  ...
  describe("Transaction queue", () => {
    ...
    it("rejects transaction if queue already contains enough operation from same account", async () => {
      const user = TestUser();
      ...
      const promise = _connection.client.signAndSendUniqueTransaction(
        {
          operations: [
            ftAuth(account.id, deriveAuthDescriptorId(user.authDescriptor)),
            op("test_authenticated_operation"),
      ...
      promise.on("sent", async (event) => {
        expect(event.status).toBe(ResponseStatus.Waiting);
        await expect(
          _connection.client.signAndSendUniqueTransaction(
            {
              operations: [
