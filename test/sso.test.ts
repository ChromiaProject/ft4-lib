...
describe("SSO", () => {
    ...
    it("should create account", async () => {
        ...
        const [account, user] = await sso.finalizeLogin(rawTransaction.toString('hex'));
    ...
    it("should throw an error transaction has more than 2 operations", async () => {
        ...
        const rawTransaction = blockchain.transactionBuilder()
            ...
            .add(addAuthDescriptor(vaultUser.authDescriptor.id, vaultUser.authDescriptor.id, dappUser.authDescriptor))
            .add(nop())
            .build([vaultUser.authDescriptor.signers, dappUser.authDescriptor.signers].flat())
    ...
    it("should throw an error if there is one operation and it isn't add_auth_descriptor", async () => {
        ...
        await expect(promise).rejects.toThrowError("Expected 'ft3.add_auth_descriptor'");
    });
    ...
    it("should throw an error if there are two operations and first is not register", async () => {
        ...
        await expect(promise).rejects.toThrowError("Expected 'ft3.dev_register_account'");
    });
