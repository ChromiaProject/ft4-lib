...
export async function createOrchestrator(
  ...
  if (!asset) {
    throw new OrchestratorError(`No asset found with id: ${assetId}`);
  }
  ...
  async function applyTransfer(
    ...
    if (!state.tx) {
      throw new OrchestratorError("No transaction to apply");
    }
  ...
  async function createIccfProof(
    ...
    if (!state.tx) {
      throw new OrchestratorError("No transaction to create ICCF proof for");
    }
