...
class Account {
  ...
  async deleteAllAuthDescriptorsExclude(
    ...
    await this.session.call(
      deleteAllAuthDescriptorsExclude(this.id, authDescriptor.id)
    );
    this.authDescriptor = [authDescriptor];
  }
  ...
  private async syncAssets(): Promise<void> {
    this.assets = await AssetBalance.getByAccountId(
      this.id,
      this.session.blockchain
    );
  }
  ...
  async getPaymentHistory(): Promise<any[]> {
    return await PaymentHistory.getByAccountId(
      this.id,
      -1,
      this.session.blockchain
    );
  ...
  async getPaymentHistoryIterator(pageSize): Promise<PaymentHistoryIterator> {
    ...
    await this.paymentHistorySyncManager.syncAccount(
      this.id,
      this.session.blockchain
    );
