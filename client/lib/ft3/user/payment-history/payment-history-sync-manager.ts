...
let defaultPaymentHistoryStore: PaymentHistoryStore =
  new PaymentHistoryStoreMemory();
...
export default class PaymentHistorySyncManager {
  readonly paymentHistoryStore: PaymentHistoryStore =
    defaultPaymentHistoryStore || new PaymentHistoryStoreNullObject();
  ...
  async syncAccount(id: Buffer, blockchain: Blockchain) {
    ...
    const paymentHistory = await PaymentHistory.getByAccountId(
      id,
      lastBlock,
      blockchain
    );
    ...
    const paymentHistoryEntries = this.mapShortEntriesToLongEntries(
      paymentHistory,
      blockchain.id,
      id
    );
  ...
  private groupShortEntriesByTransactionRID(
    entries: PaymentHistoryEntryShort[]
  ): Map<string, any[]> {
    const entriesGroups: Map<string, PaymentHistoryEntryShort[]> = new Map<
      string,
      PaymentHistoryEntryShort[]
    >();
  ...
  private paymentHistoryEntriesFrom(
    ...
    for (const entry of entries) {
      if (entry.isInput) {
        const input = this.matchPaymentHistoryEntryAndPaymentParam(
          entry,
          inputs,
          accountId
        );
        if (!input) {
          throw new Error(
            "Cannot match payment history entry to any transfer input"
          );
        }
        inputs = inputs.filter((i) => i !== input);
        paymentHistoryEntries.push(
          this.getPaymentHistoryEntry(entry, input.payment)
        );
      } else {
        const output = this.matchPaymentHistoryEntryAndPaymentParam(
          entry,
          outputs,
          accountId
        );
        if (!output) {
          throw new Error(
            "Cannot match payment history entry to any transfer output"
          );
        }
        outputs = outputs.filter((o) => o !== output);
        paymentHistoryEntries.push(
          this.getPaymentHistoryEntry(entry, output.payment)
        );
      }
    }
  ...
  private matchPaymentHistoryEntryAndPaymentParam(
    entry: PaymentHistoryEntryShort,
    params: ParamPaymentPair[],
    accountId: Buffer
  ): ParamPaymentPair {
    return params.find(
      (param) =>
