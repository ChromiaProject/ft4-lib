...
function isCustomPaymentOperation(operation) {
    ...
    if (!Array.isArray(lastArgument) || lastArgument.length === 0) { return false }
    ...
    extract(): PaymentOperation[] {
        ...
        return transaction.operations.map(operation => {
            switch (operation.opName) {
                case 'ft3.transfer':
                    return PaymentOperation.fromTransfer(TransferOperation.from(operation), this.chainId);
                case 'ft3.xc.init_xfer':
                    return PaymentOperation.fromXTransfer(XTransferOperation.from(operation), this.chainId);
                default:
