...
```sh
docker run --name postchain -e POSTGRES_INITDB_ ARGS="--lc-collate=C.UTF-8 \
    --lc-ctype=C.UTF-8 --encoding=UTF-8" -e POSTGRES_PASSWORD=postchain \
    -e POSTGRES_USER=postchain -p 5432:5432 postgres
```
...
```sh
cd examples/metamask-svelte-example
