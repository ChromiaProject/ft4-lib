...
- use docker-compose file from `test-docker` directory to start up a database preconfigured to work with Rell if you don't have it already
- run  `./postchain/bin/run-node.sh test --wipe-db` and keep it running
...
- run `npm test [-t testName]` to run tests (optional: only the tests matching the regex after -t)
